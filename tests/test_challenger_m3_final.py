...
class TestLifespanAndResourceCleanupChallenger:
    ...
    @pytest.mark.anyio
    async def test_lifespan_rapid_cycles_with_active_tasks(self, tmp_path: Path) -> None:
        ...
        for cycle in range(5):
            ...
            assert len(server.active_agent_tasks) == 0, f"Cycle {cycle} leaked agent tasks"
    ...
    def test_wakeword_stop_latency_invariant(self) -> None:
        ...
        with patch("src.speech.wake_word.Model"), \
            ...
            assert elapsed < 0.5, f"WakeWordDetector stop took {elapsed:.3f}s (expected < 0.5s)"
...
class TestTTSAudioSuppressionChallenger:
    ...
    def test_tts_disabled_settings_bypass(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestConcurrencyBurstChallenger:
    ...
    @pytest.mark.anyio
    async def test_high_concurrency_settings_and_workspace_io(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        for res in results:
            assert not isinstance(res, Exception), f"Concurrent settings I/O failed: {res}"
