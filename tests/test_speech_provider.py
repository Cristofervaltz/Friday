...
def test_google_provider_successful_transcription():
    ...
    with patch("src.speech.google_provider.sr") as mock_sr:
        class DummyWaitTimeoutError(Exception): pass
        class DummyUnknownValueError(Exception): pass
        ...
        provider.recognizer.recognize_google.assert_called_once_with(fake_audio, language="en-US")
...
def test_google_provider_abort_closes_stream():
    ...
    with patch("src.speech.google_provider.sr") as mock_sr:
        ...
        with pytest.raises(RuntimeError, match="Voice recording aborted by user"):
            provider.listen_and_transcribe(timeout=5, phrase_time_limit=5, abort_event=abort_event)
