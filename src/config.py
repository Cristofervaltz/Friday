...
class AppConfig:
    ...
    def from_environment(cls, base_dir: Path | None = None) -> AppConfig:
        ...
        def get_val(key: str, env_key: str, default: str | None = None) -> str | None:
            return saved_settings.get(key, getenv(env_key, default))

        configured_log_dir = Path(get_val("log_dir", "FRIDAY_LOG_DIR") or str(paths.logs_dir))
        logging_config = LoggingConfig(
            level=get_val("log_level", "FRIDAY_LOG_LEVEL", DEFAULT_LOG_LEVEL).upper(),
            log_dir=configured_log_dir.expanduser().resolve(),
            log_filename=get_val("log_filename", "FRIDAY_LOG_FILENAME", DEFAULT_LOG_FILENAME),
            max_bytes=int(get_val("log_max_bytes", "FRIDAY_LOG_MAX_BYTES", "1048576")),
            backup_count=int(get_val("log_backup_count", "FRIDAY_LOG_BACKUP_COUNT", "5")),
            console_enabled=str(get_val("console_logging", "FRIDAY_CONSOLE_LOGGING", "true")).lower()
            in {"1", "true", "yes", "on"},
            file_enabled=str(get_val("file_logging", "FRIDAY_FILE_LOGGING", "true")).lower()
            in {"1", "true", "yes", "on"},
        )
...
def save_settings(settings: dict, base_dir: Path | None = None) -> None:
    resolved_base_dir = base_dir or Path.cwd()
    paths = PathsConfig.from_base_dir(resolved_base_dir)
