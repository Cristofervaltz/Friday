...
class ConversationMemory:
    ...
    def add_assistant_message(
        ...
        """
        import uuid
        msg: dict[str, Any] = {"id": uuid.uuid4().hex, "role": "assistant", "content": content}
    ...
    def add_message(self, role: str, content: str, chat_id: str | None = None) -> None:
        ...
        if role == "user":
            ...
            with self._lock:
                self._messages.append({"id": uuid.uuid4().hex, "role": "system", "content": content})
                self._enforce_max_messages()
                self._save_unsafe()
            self._trigger_on_change()
    ...
    def truncate_messages(self, message_id: str, inclusive: bool = False) -> None:
        ...
        with self._lock:
            idx = next((i for i, m in enumerate(self._messages) if m.get("id") == message_id), -1)
            if idx != -1:
