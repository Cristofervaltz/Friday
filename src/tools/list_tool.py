    ...
    def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            if not dir_path.exists():
                return ToolResult(
                    success=False, error=f"Directory not found: {path_str}"
                )

            # Security: check if it's a directory
            if not dir_path.is_dir():
                return ToolResult(
                    success=False, error=f"Path is not a directory: {path_str}"
                )
            ...
            if sort_by == "size":
                entries.sort(
                    key=lambda e: e.stat().st_size if e.is_file() else 0, reverse=True
                )
            elif sort_by == "date":
            ...
            if not entries:
                ...
                for entry in entries:
                    output_lines.append(
                        self._format_entry(entry, dir_path, show_details)
                    )
